...
import { BASE_URL, PAYPAL_CLIENT_ID } from '../../utils/constants';
...
const ShippingPage = () => {
    ...
    useEffect(() => {
        APIinstance.get(`store/checkout/${order_oid}`).then((response) => {
    ...
    const payWithStripe = async (e) => {
        ...
        e.preventDefault();
        APIinstance.post(`store/stripe-checkout/${order.oid}/`).then((response) => {
            const stripe = loadStripe(response.data.stripe_public_key);
    ...
    <>
        <div className='flex flex-col w-2/3 border-r h-screen pl-96 pr-56 pt-24 items-center'>
            ...
            <div className='flex mb-4'>
                <ol className='inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse'>
                    <li className='inline-flex items-center'>
                      <span className="ms-1 text-xs font-small text-white md:ms-2 dark:text-gray-400 uppercase">cart</span>
                    </li>
                    <li>
                        <div className="flex items-center">
                            ...
                            <span className="ms-1 text-xs font-small text-white md:ms-2 dark:text-gray-400 uppercase">Information</span>
                        </div>
                    </li>
                    <li>
                        <div className="flex items-center">
                            ...
                            <span className="ms-1 text-xs font-small text-gray-700 md:ms-2 dark:text-white uppercase">Shipping</span>
                        </div>
                    </li>
                    <li>
                        <div className="flex items-center">
            ...
            <form className="w-full">
                ...
                <div className="flex items-center justify-between w-full mb-2">
                    <button className=''>
                        <Link to="/checkout" className="text-gray-500 hover:text-gray-900 dark:hover:text-white">Back to Information</Link>
                    </button>
                </div>
            </form>
        ...
        <div className='w-1/3'>
            <div className='fixed top-0 right-0 z-40 h-screen overflow-y-auto transition-transform translate-x-0 bg-white w-1/3 dark:bg-black' tabIndex="-1" aria-labelledby="drawer-right-label">
                <div className="flex flex-col items-center justify-between h-full pt-12 pl-12 pr-12">
                    <div className="">
                        ...
                        <div>
                            <div className="flex mb-2 items-center justify-between">
                                ...
                                <p className="text-sm text-gray-500 dark:text-gray-400">$ {order.sub_total}</p>
                            </div>
                            <div className="flex mb-2 items-center justify-between">
                                <h6 className="text-sm font-semibold text-gray-900 dark:text-gray-300 uppercase">Shipping</h6>
                                <p className="text-sm text-gray-500 dark:text-gray-400">$ {order.shipping_amount}</p>
                            </div>
                            <div className="flex mb-2 items-center justify-between">
                                <h6 className="text-sm font-semibold text-gray-900 dark:text-gray-300 uppercase">Tax</h6>
                                <p className="text-sm text-gray-500 dark:text-gray-400">$ {order.tax_fee}</p> 
                            </div>
                    ...
                    <div className='mb-52'>
                    {paymentLoading ? (
                        <form action={`${BASE_URL}stripe-checkout/${order?.oid}/`}>
                            <button disabled type="button" className="py-2.5 px-5 me-2 text-sm font-medium text-gray-900 bg-white rounded-sm border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 inline-flex items-center">
                        ...
                        <form action={`${BASE_URL}stripe-checkout/${order?.oid}/`} method='POST'>
                            <button onClick={payWithStripe} type="button" className="py-2.5 px-5 me-2 text-sm font-medium text-gray-900 bg-white rounded-sm border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 inline-flex items-center">
                                <svg className="w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <path fill="currentColor" d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm0 22c-5.522 0-10-4.478-10-10s4.478-10 10-10 10 4.478 10 10-4.478 10-10 10zm-1-15h2v6h-2v-6zm0 8h2v2h-2v-2z"/>
                                </svg>
                                Pay with Stripe
                            </button>
                        </form>
